...
from settings.network_settings.network_settings import DEFAULT_PORT, update_host_address, NETWORK_DATA, anon_host, IP, \
    PASSWORD
from settings.common_settings import PLAYER_NICKNAME_KEY
...
def connect_as_client():
    if NETWORK_DATA[IP]:
        Stages().set_multiplayer_client_connect_round_stage()
    else:
        # TODO ERROR MESSAGE
        pass
...
def nick_change():
    ...
    if new_nick_name:
        save_param_to_cgs(PLAYER_NICKNAME_KEY, new_nick_name)
...
NICKNAME_INPUT = InputElement(x=HALF_SCREEN_W + DEFAULT_BUTTON_X_SIZE + int(50 * Y_SCALE),
                              ...
                              size_x=200,
                              text=get_param_from_cgs(key=PLAYER_NICKNAME_KEY,
                                                                  def_value=''),
...
SERVER_PASSWORD = InputElement(x=250 * X_SCALE, y=100 * Y_SCALE, default_text='Enter password',
                               on_change_action=password_change)
...
MULTIPLAYER_BUTTONS = {
    ...
    '_host': {
        'kwargs': {
            ...
            'size_x': 200,
            'text': 'RUN SEVER',
            'on_click_action': Stages().set_host_server_stage,
        }
    },
