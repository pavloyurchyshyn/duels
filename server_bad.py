...
class Server:
    ...
    def run(self):
        ...
        while self.socket_opened:
            ...
            LOGGER.info(
                f"Network_address_key {network_acc_k} in {self.player_connected_in_past} is {credentials.get('network_address_key') in self.player_connected_in_past}")

            if network_acc_k in self.player_connected_in_past:
                # =================== RECONNECT ======================
                LOGGER.info(f'Player {anon_host(addr)} is reconnecting.')
                server_response_data['connected'] = True

                player: NetworkPlayerObject = self.players_objects[network_acc_k]

                server_response_data['position'] = player.position
                server_response_data['angle'] = player.angle
                server_response_data['hp'] = player.health_points
                server_response_data['dead_addr_list'] = self.dead_list

                if DELETE_PLAYER in self.server_actions:
                    self.server_actions[DELETE_PLAYER].append(network_acc_k)
                else:
                ...
                self.player_connected_in_past.remove(network_acc_k)
                ...
                if credentials[PASSWORD] == self.password:
                    ...
                    player = NetworkPlayerObject(*self.SERVER_GAME.get_spawn_pos(team), team=team)
                    self.players_objects[str(addr)] = player
    ...
    def disconnect_player(self, addr, del_player=False):
        # todo del_player -> ban_player
        LOGGER.info(f'Disconnecting player {addr}')

        try:
            # try:
            #     self.send(addr, sock=self.players_connections[addr], data=self.json_to_str({'disconnect': True}))
            # except Exception as e:
            #     LOGGER.error(f"Failed to send disconnect command to client: \n\t {e}")
            self.dead_list.add(addr)
        ...
        LOGGER.info(
            f'Player {addr} deleted from {tuple(self.players_connections.keys())} to {self.player_connected_in_past}.')

        if str(addr) in self.players_connections:
            self.players_connections.pop(str(addr))
        self.player_connected_in_past.append(str(addr))
    ...
    def disconnect_all_players(self):
        for connect in self.players_connections.copy():
    ...
    def stop_server(self):
        ...
        try:
            ...
            if self.game_thread_id:
                os.kill(self.game_thread_id, signal.SIGTERM)
        except Exception:
            pass
...
class ServerGame:
    ...
    def __init__(self, players_connections, server,
        ...
        self.teams = teams  # teams with all players
        ...
        self.ARENA = ArenaCellObject(data=cell_data)

        self.spawn_positions = {
            'red': set((x, y) for x in range(50, 100, 50) for y in range(0, self.ARENA.size, 50)),
            'blue': set((x, y) for x in range(self.ARENA.size - 50, self.ARENA.size, 50) for y in
                        range(0, self.ARENA.size, 50)),
        }
    ...
    def WAITING_FOR_PLAYERS(self):
        ...
        for addr, conn in self.players_connections.copy().items():
            if addr in self.server.dead_list:
                continue
            ...
            try:
                ...
                if data:
                    ...
                    player: NetworkPlayerObject = self.players_objects[str(addr)]
                    if player.alive:
        ...
        if self.server.server_actions or self.clock.time > 0:
    ...
    def PREPARING_TO_ROUND(self):
        ...
        for addr, player in self.players_objects.copy().items():
            while 1:
    ...
    def is_any_team_won(self):
        teams = {team: 0 for team in self.teams if team != 'spectator'}
        for player in self.players_objects.values():
            teams[player.team] += player.health_points

        dead_teams = [team for team, hp in teams.items() if hp <= 0]
        if dead_teams:
            if len(teams) - len(dead_teams) == 1:
                team = tuple(dead_teams)[0]
                LOGGER.info(f"TEAM {team.upper()} WON")
                self.data_to_send[ROUND_WINNER] = f"TEAM {team.upper()} WON"
